...
public class Archive extends AbstractAction {
    ...
    private void addToArchive(final String filePath, final String parent, final TarArchiveOutputStream tarArchive) throws IOException {
        ...
        if (file.isFile()) {
            ...
            for (java.io.File f : Objects.requireNonNull(file.listFiles())) {
                // recursive call
                addToArchive(f.getAbsolutePath(), entryName + java.io.File.separator, tarArchive);
            }
        }
    }
    ...
    private void unTar(String archivePath, String folderPath, java.io.File destFile, TarArchiveInputStream tis) throws IOException {
    ...
    private void unTarGzFile(final String archivePath, final String folderPath) {
        java.io.File destFile = Paths.get(folderPath).toFile();
